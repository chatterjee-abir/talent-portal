...
// Historical cert trend — stored as static data (not in DB)
const CERT_TREND = [
...
function Spinner() {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center",
                  padding:64, color:"var(--text-muted)", gap:10 }}>
      <div style={{ width:20, height:20, border:"3px solid var(--border2)",
...
// Re-fetches any time `url` changes.
function useApi(url) {
...
// ═══════════════════════════════════════════
function SummaryTab() {
  const { data: summary, loading: lS  } = useApi(`${API}/dashboard/summary`);
  const { data: skills,  loading: lSk } = useApi(`${API}/skills`);
  const { data: assocs,  loading: lA  } = useApi(`${API}/associates`);

  if (lS || lSk || lA) return <Spinner />;

  // ── KPIs ─────────────────────────────────
  const totalAssoc = summary?.totalAssociates     ?? 0;
  const certified  = summary?.certifiedAssociates ?? 0;
  const level3Plus = summary?.level3PlusSkills    ?? 0;
  const avgSkill   = summary?.avgSkillLevel       ?? 0;
  const improved   = summary?.skillsImproved      ?? 0;

  // ── Skill chart data ──────────────────────
  const skillNames = (skills ?? []).map(s => s.skillName);
  const skillAvgs  = (skills ?? []).map(s => +s.averageLevel);
  const level3Cnts = (skills ?? []).map(s => +s.level3PlusCount);
  const weakAreas  = (skills ?? []).filter(s => s.averageLevel < 3).map(s => s.skillName);

  // Previous-month averages: derive as (currentLevel - delta) per associate
  const prevAvgs = SKILLS.map(sName => {
    const vals = (assocs ?? []).flatMap(a =>
      (a.skills ?? []).filter(sk => sk.skillName === sName)
                      .map(sk => sk.currentLevel - (sk.delta ?? 0))
    );
  ...
  const depts = {};
  (assocs ?? []).forEach(a => { depts[a.department] = (depts[a.department] || 0) + 1; });
  ...
  const skillBarOpts = {
    ...
    series:[
      { name:"This Month", data: skillAvgs },
      { name:"Last Month", data: prevAvgs  },
    ],
    xaxis:{ categories: skillNames, labels:{ style:{ fontSize:"12px" } } },
  ...
  const level3Opts = {
    chart:{ type:"bar", toolbar:{show:false}, fontFamily:"IBM Plex Sans" },
    series:[{ name:"Associates at Level 3+", data: level3Cnts }],
    xaxis:{ categories: skillNames, labels:{ style:{ fontSize:"12px" } } },
    yaxis:{ min:0, max:55 },
    colors:["#00875a"],
    plotOptions:{ bar:{ borderRadius:4, horizontal:true, barHeight:"55%", dataLabels:{ position:"right" } } },
  ...
  const deptPieOpts = {
    ...
    plotOptions:{ pie:{ donut:{ size:"62%",
      labels:{ show:true, total:{ show:true, label:"Total", color:"#172b4d",
        fontSize:"13px", fontWeight:700, formatter:() => String(totalAssoc) },
    ...
    <div>
      {weakAreas.length > 0 && (
        <div className="alert-banner alert-warn">
          ⚠️ <strong>Weak Areas Detected:</strong> {weakAreas.join(", ")} — average level below 3.0.
        </div>
      )}
      <div className="alert-banner alert-success">
        📈 <strong>Progress Update:</strong> {improved} skills improved this period. {certified} associates are certified.
      </div>
      ...
      <div className="kpi-grid">
        <div className="kpi-card blue">
          ...
          <div className="kpi-delta up">↑ Active team</div>
        </div>
        <div className="kpi-card green">
          <div className="kpi-icon">🎓</div>
          <div className="kpi-label">Certified</div>
          <div className="kpi-value">{certified}</div>
          <div className="kpi-delta up">{totalAssoc ? Math.round(certified/totalAssoc*100) : 0}% of team</div>
        </div>
        <div className="kpi-card purple">
          <div className="kpi-icon">⭐</div>
          <div className="kpi-label">Level 3+ Skills</div>
          <div className="kpi-value">{level3Plus}</div>
          <div className="kpi-delta up">across all skills</div>
        </div>
        <div className="kpi-card orange">
          <div className="kpi-icon">📊</div>
          <div className="kpi-label">Avg Skill Level</div>
          <div className="kpi-value">{avgSkill}</div>
          <div className="kpi-delta up">across 5 skills</div>
        </div>
        <div className="kpi-card blue">
          <div className="kpi-icon">🚀</div>
          <div className="kpi-label">Skills Improved</div>
          <div className="kpi-value">{improved}</div>
          <div className="kpi-delta">this period</div>
        </div>
      </div>

      <div className="charts-grid">
        <div className="card"><div className="card-body chart-wrap">
          <ApexChart id="apex-skill-bar" options={skillBarOpts} height={280} />
        </div></div>
        <div className="card"><div className="card-body chart-wrap">
          <ApexChart id="apex-level3" options={level3Opts} height={280} />
        </div></div>
        <div className="card"><div className="card-body chart-wrap">
          <ApexChart id="apex-cert-trend" options={certTrendOpts} height={280} />
        </div></div>
        <div className="card">
          <div className="card-body" style={{ display:"flex", alignItems:"center" }}>
            <div style={{ flex:"0 0 55%" }}>
              <ApexChart id="apex-dept-pie" options={deptPieOpts} height={280} />
            </div>
              ...
              {deptEntries.map(([dept, count], idx) => (
                <div key={dept} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                  <div style={{ width:12, height:12, borderRadius:3,
                                background:DEPT_COLORS[idx % DEPT_COLORS.length], flexShrink:0 }} />
                  <div style={{ flex:1, fontSize:13, fontWeight:500 }}>{dept}</div>
                  <div style={{ fontSize:13, fontFamily:"var(--mono)", fontWeight:700 }}>{count}</div>
                  <div style={{ fontSize:11, color:"var(--text-muted)", minWidth:32 }}>
                    {totalAssoc ? Math.round(count/totalAssoc*100) : 0}%
                  </div>
                </div>
              ))}
            </div>
...
function LearningTab({ globalEmpId, globalName }) {
  ...
  useEffect(() => {
    setEmpId(globalEmpId || ""); setName(globalName || "");
    setActiveFilters(f => ({ ...f, empId: globalEmpId || "", name: globalName || "" }));
    setPage(1);
  }, [globalEmpId, globalName]);

  // Pass empId / name to backend; skill+minLevel filtered client-side
  const qs = useMemo(() => {
    const p = new URLSearchParams();
    if (activeFilters.empId) p.set("empId", activeFilters.empId);
    if (activeFilters.name)  p.set("name",  activeFilters.name);
    return p.toString();
  }, [activeFilters.empId, activeFilters.name]);

  const { data: rawAssocs, loading } = useApi(`${API}/associates${qs ? "?" + qs : ""}`);

  const filtered = useMemo(() => {
    if (!rawAssocs) return [];
    const { skill: sk, minLevel: ml } = activeFilters;
    const lvl = +ml;
    return rawAssocs.filter(a => {
      const lv = sName => a.skills?.find(s => s.skillName === sName)?.currentLevel ?? 0;
      if (sk !== "All" && lvl > 1 && lv(sk) < lvl) return false;
      if (sk === "All" && lvl > 1 && !SKILLS.some(s => lv(s) >= lvl)) return false;
      return true;
    });
  }, [rawAssocs, activeFilters]);

  const paged = filtered.slice((page-1)*LEARNING_PER_PAGE, page*LEARNING_PER_PAGE);

  const handleSearch = () => { setActiveFilters({ empId, name, skill, minLevel }); setPage(1); };
  const handleReset  = () => {
    setEmpId(""); setName(""); setSkill("All"); setMinLevel("1");
    setActiveFilters({ empId:"", name:"", skill:"All", minLevel:"1" }); setPage(1);
  };
  ...
  return (
    <div>
      <div className="search-row">
        <label>Filter:</label>
        <input className="search-input" placeholder="Employee ID" value={empId}
          onChange={e => setEmpId(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} />
        <input className="search-input" placeholder="Associate Name" value={name}
          onChange={e => setName(e.target.value)} onKeyDown={e => e.key==="Enter" && handleSearch()} />
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          <label>Skill:</label>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          ...
          <select className="select-field" style={{ minWidth:80 }} value={minLevel}
        ...
        <>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Emp ID</th><th>Associate Name</th><th>Dept</th>
              ...
              <tbody>
                {paged.length === 0 ? (
                  <tr><td colSpan={SKILLS.length+4} style={{ textAlign:"center", padding:32, color:"var(--text-muted)" }}>
                    No associates match your search.
                  </td></tr>
                ) : paged.map(a => {
                  const getsk = sName => a.skills?.find(s => s.skillName === sName) ?? { currentLevel:1, previousLevel:1 };
                  const avg = (SKILLS.reduce((sum,s) => sum + getsk(s).currentLevel, 0) / SKILLS.length).toFixed(1);
                  return (
                    <tr key={a.empId}>
                      ...
                      {SKILLS.map(s => {
                        const sk = getsk(s);
                        return <td key={s}><LevelBar current={sk.currentLevel} previous={sk.previousLevel} /></td>;
                      })}
                      <td><span className={`badge ${levelBadgeClass(Math.round(+avg))}`}>{avg}</span></td>
                    </tr>
                  );
                })}
              </tbody>
...
function CertificationTab({ globalEmpId, globalName }) {
  ...
  useEffect(() => {
    setActiveEmpId(globalEmpId || ""); setActiveName(globalName || ""); setPage(1);
  }, [globalEmpId, globalName]);

  const { data: certsData, loading: lC } = useApi(`${API}/certifications`);
  const { data: allAssocs, loading: lA } = useApi(`${API}/associates`);

  const certCounts = useMemo(() =>
    (certsData ?? []).map(c => ({ cert: c.name, count: c.holderCount }))
                     .sort((a,b) => b.count - a.count),
  [certsData]);

  const allCertNames = useMemo(() =>
    (certsData ?? []).map(c => c.name).sort(),
  [certsData]);

  const certifiedAssocs = useMemo(() =>
    (allAssocs ?? []).filter(a => (a.certifications?.length ?? 0) > 0),
  [allAssocs]);

  const filtered = useMemo(() => (allAssocs ?? []).filter(a => {
    if (activeCert !== "All" && !a.certifications?.includes(activeCert)) return false;
    if (activeEmpId && !a.empId.toLowerCase().includes(activeEmpId.toLowerCase())) return false;
    if (activeName  && !a.fullName.toLowerCase().includes(activeName.toLowerCase())) return false;
    return true;
  }), [activeCert, activeEmpId, activeName, allAssocs]);
  ...
  const handleReset  = () => {
    setSelectedCert("All"); setEmpId(""); setName("");
    setActiveCert("All"); setActiveEmpId(""); setActiveName(""); setPage(1);
  };
  ...
  if (lC || lA) return <Spinner />;

  const total = allAssocs?.length ?? 50;
  ...
  return (
    <div>
        ...
        <div className="card">
          <div className="card-body" style={{ display:"flex", gap:24, alignItems:"center" }}>
            <div style={{ textAlign:"center" }}>
              <div style={{ fontSize:48, fontWeight:700, color:"var(--accent)", fontFamily:"var(--mono)" }}>
                {certifiedAssocs.length}
              </div>
              <div style={{ fontSize:12, color:"var(--text-muted)" }}>Associates Certified</div>
              <div style={{ fontSize:12, color:"var(--success)", marginTop:4 }}>
                ▲ {CERT_TREND[CERT_TREND.length-1].count - CERT_TREND[CERT_TREND.length-2].count} this month
              </div>
            </div>
            <div style={{ flex:1 }}>
              {certCounts.slice(0,5).map((c,i) => (
                <div key={c.cert} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                  ...
                  <div style={{ flex:1, fontSize:12, fontWeight:500 }}>{c.cert}</div>
                  <div style={{ width:`${Math.round(c.count/total*100)}%`, maxWidth:80, height:6,
                                background:"var(--accent)", borderRadius:3, minWidth:4 }} />
                  <div style={{ fontSize:12, fontFamily:"var(--mono)", color:"var(--text-secondary)", minWidth:20 }}>
                    {c.count}
                  </div>
                </div>
              ))}
      ...
      <div className="card" style={{ marginBottom:20 }}>
        <div className="card-body chart-wrap">
          <ApexChart id="apex-cert-bar" options={certBarOpts} height={240} />
        </div>
      </div>
      ...
      <div className="search-row">
        ...
        <select className="select-field" value={selectedCert} onChange={e => setSelectedCert(e.target.value)}>
          ...
          {allCertNames.map(c => <option key={c}>{c}</option>)}
        </select>
        <input className="search-input" placeholder="Employee ID" value={empId}
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Emp ID</th><th>Associate Name</th><th>Dept</th>
              <th># Certs</th><th>Certifications Held</th>
            </tr>
          </thead>
          <tbody>
            {paged.length === 0 ? (
              ...
              return (
                <tr key={a.empId}>
                  ...
                  <td style={{ fontWeight:500 }}>{a.fullName}</td>
                  <td><span className="badge badge-blue">{a.department}</span></td>
                  <td><span className={`badge ${certs.length >= 3 ? "badge-green" : certs.length >= 1 ? "badge-orange" : "badge-grey"}`}>
                    {certs.length}
                  </span></td>
                  <td>
                    <div className="cert-pills">
                      {certs.length === 0
                        ...
                        : certs.map(c => <span key={c} className="badge badge-purple" style={{ marginBottom:2 }}>{c}</span>)
                      }
                    </div>
                  </td>
                </tr>
...
function ProjectTab({ globalEmpId, globalName }) {
  ...
  // Charts — always full dataset
  const { data: projectsData } = useApi(`${API}/projects`);
  const { data: allAssign    } = useApi(`${API}/projects/assignments`);

  // Filtered assignments — pass project+name to backend, empId+role client-side
  const qs = useMemo(() => {
    const p = new URLSearchParams();
    if (activeProjId !== "All") p.set("projectCode", activeProjId);
    if (activeName)             p.set("name",        activeName);
    return p.toString();
  }, [activeProjId, activeName]);

  const { data: assignData, loading } = useApi(`${API}/projects/assignments${qs ? "?" + qs : ""}`);

  const filtered = useMemo(() => (assignData ?? []).filter(r => {
    if (activeEmpId && !r.empId.toLowerCase().includes(activeEmpId.toLowerCase())) return false;
    if (activeRole !== "All" && r.role !== activeRole) return false;
    return true;
  }), [assignData, activeEmpId, activeRole]);

  const paged = filtered.slice((page-1)*PROJ_PER_PAGE, page*PROJ_PER_PAGE);

  const handleSearch = () => {
    setActiveProjId(projId); setActiveEmpId(empId); setActiveName(name); setActiveRole(role); setPage(1);
  };
  const handleReset = () => {
    setProjId("All"); setEmpId(""); setName(""); setRole("All");
    setActiveProjId("All"); setActiveEmpId(""); setActiveName(""); setActiveRole("All"); setPage(1);
  };

  // Chart data
  const projSize = (projectsData ?? []).map(p => ({ name: p.projectName, count: p.totalAssigned ?? 0 }));
  const projPieOpts = {
    chart:{ type:"donut", fontFamily:"IBM Plex Sans", toolbar:{show:false} },
  ...
  const roleData = ROLES.map(r => (allAssign ?? []).filter(a => a.role === r).length);
  const roleBarOpts = {
    chart:{ type:"bar", toolbar:{show:false}, fontFamily:"IBM Plex Sans" },
  ...
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:20 }}>
        <div className="card"><div className="card-body chart-wrap">
          <ApexChart id="apex-proj-pie" options={projPieOpts} height={260} />
        </div></div>
        <div className="card"><div className="card-body chart-wrap">
          <ApexChart id="apex-role-bar" options={roleBarOpts} height={260} />
        </div></div>
      </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Emp ID</th><th>Associate Name</th>
                  <th>Role</th><th>Start Date</th><th>End Date</th><th>Status</th>
                </tr>
              </thead>
              <tbody>
                {paged.length === 0 ? (
                  ...
                  <tr key={i}>
                    <td><span className="badge badge-blue" style={{ fontFamily:"var(--mono)" }}>{r.projectCode}</span></td>
                    <td style={{ fontWeight:500 }}>{r.projectName}</td>
                    <td><span className="badge badge-grey" style={{ fontFamily:"var(--mono)" }}>{r.empId}</span></td>
                    <td>{r.fullName}</td>
                    ...
                    <td><span className={`badge ${r.status === "Active" ? "badge-green" : "badge-grey"}`}>
                      {r.status}
                    </span></td>
                  </tr>
                ))}
